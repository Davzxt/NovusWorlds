...
export async function renderHeader() {
  ...
  host.innerHTML = `
    <div class="topbar"><div class="wrap nav">
      ...
      <div class="userbox">${user ? `<span class="novux">N$ ${user.novux}</span><a class="btn secondary" href="/profile.html?user=${encodeURIComponent(user.username)}">${user.username}</a><a class="btn secondary" href="/avatar.html">Avatar</a><a class="btn" href="/studio-dashboard.html">Studio</a>${user.is_admin ? '<a class="btn secondary" href="/admin/index.html">Admin</a>' : ''}<button id="logoutBtn" class="danger">Sair</button>` : '<a class="btn secondary" href="/login.html">Entrar</a><a class="btn" href="/register.html">Registrar</a>'}</div>
    </div></div>`;
