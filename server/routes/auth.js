  ...
  try {
    ...
    if (user.is_banned === 1) {
      ...
      if (user.ban_expires && new Date(user.ban_expires) < new Date()) {
        prepare('UPDATE users SET is_banned = 0, ban_reason = NULL, ban_expires = NULL WHERE id = ?').run(user.id);
      } else {
    ...
    if (lastLogin !== today) {
      dailyBonus = 10;
      prepare('UPDATE users SET novux = novux + 10 WHERE id = ?').run(user.id);
      prepare(`
        INSERT INTO transactions (from_user_id, to_user_id, amount, type, description)
